...
| `deploy [--systemd\|--tmux]` | Generate VPS deployment scripts for the fleet |
| `send <to> <message>` | Send a message to another bot |
...
Generate deployment scripts for running the bot fleet on a VPS or remote server.

1. Read `crew-registry.json` — if no bots, suggest `/solocrew create`
2. Ask which deployment mode:
   - **tmux** (default) — single launch script that creates a tmux session with all bots
   - **systemd** — one `.service` unit file per bot for systemd-managed processes
3. Create output directory:
   ```bash
   mkdir -p ~/.claude/channels/crew-deploy
   chmod 700 ~/.claude/channels/crew-deploy
   ```

4. **tmux mode** — Generate `crew-launch.sh`:
   - Creates a tmux session named `crew`
   - Adds a window per bot with the FULL alias command (not alias name — tmux doesn't load RC files)
   - Includes `--model <model>` flag if the bot has a model set in the registry
   - Safe mode by default (`--dangerously-skip-permissions` is NOT included)
   - Includes a commented-out `# AUTONOMOUS MODE` section per bot with the `--dangerously-skip-permissions` flag, so the operator can opt in per bot
   - Example structure:
     ```bash
     #!/usr/bin/env bash
     set -euo pipefail
     # crew-launch.sh — generated by solocrew deploy
     # Launch all bots in a tmux session
     ...
     # Bot: devbot — App development
     tmux new-session -d -s "$SESSION" -n "devbot"
     tmux send-keys -t "$SESSION:devbot" \
       'TELEGRAM_STATE_DIR=~/.claude/channels/crew-devbot claude --channels plugin:telegram@claude-plugins-official --model opus' Enter

     ## AUTONOMOUS MODE (uncomment to skip permission prompts):
     # tmux send-keys -t "$SESSION:devbot" \
     #   'TELEGRAM_STATE_DIR=~/.claude/channels/crew-devbot claude --channels plugin:telegram@claude-plugins-official --model opus --dangerously-skip-permissions' Enter

     # Bot: ops — Deployments & servers
     tmux new-window -t "$SESSION" -n "ops"
     tmux send-keys -t "$SESSION:ops" \
       'TELEGRAM_STATE_DIR=~/.claude/channels/crew-ops claude --channels plugin:telegram@claude-plugins-official --model haiku' Enter

     echo "Crew launched. Attach with: tmux attach -t $SESSION"
     ```

5. **systemd mode** — Generate one `.service` file per bot:
   - Unit runs as the current user
   - `ExecStart` is the full claude command with `TELEGRAM_STATE_DIR` and `--channels`
   - Includes `--model <model>` flag if the bot has a model set
   - `Restart=on-failure`, `RestartSec=30`
   - Environment vars for `TELEGRAM_STATE_DIR` and `CLAUDE_CODE_OAUTH_TOKEN` (placeholder — operator must fill in)
   - Example:
     ```ini
     [Unit]
     Description=solocrew bot: devbot — App development
     ...
     Type=simple
     User=%u
     Environment=TELEGRAM_STATE_DIR=%h/.claude/channels/crew-devbot
     Environment=CLAUDE_CODE_OAUTH_TOKEN=<PASTE_YOUR_OAUTH_TOKEN_HERE>
     ExecStart=/usr/local/bin/claude --channels plugin:telegram@claude-plugins-official --model opus
     Restart=on-failure
     RestartSec=30
     ...
     ```
   - Files are named `crew-<name>.service` in the deploy directory

6. Generate `crew-health-cron.sh`:
   - Checks each bot process is alive using `pgrep -f "TELEGRAM_STATE_DIR=<dir>"`
   - Sends a Telegram alert if any bot is down — uses that bot's own token (read from its `.env` file at runtime)
   - Alert message includes bot name, hostname, and timestamp
   - Designed for crontab: `*/5 * * * * ~/.claude/channels/crew-deploy/crew-health-cron.sh`
   - Example structure:
     ```bash
     #!/usr/bin/env bash
     # crew-health-cron.sh — generated by solocrew deploy
     # Check bot processes and alert via Telegram if any are down.
     # Install: crontab -e → */5 * * * * ~/.claude/channels/crew-deploy/crew-health-cron.sh

     ALERT_CHAT_ID="<owner-telegram-id>"
     HOSTNAME=$(hostname)
     ...
     check_bot() {
       ...
       if ! pgrep -f "TELEGRAM_STATE_DIR=$dir" > /dev/null 2>&1; then
         # Bot is down — send alert using its own token
         if [[ -f "$env_file" ]]; then
           local token
           token=$(grep TELEGRAM_BOT_TOKEN "$env_file" | cut -d= -f2)
           curl -s -X POST "https://api.telegram.org/bot${token}/sendMessage" \
             -d chat_id="$ALERT_CHAT_ID" \
             -d text="ALERT: Bot '$name' is DOWN on $HOSTNAME at $(date -u +%Y-%m-%dT%H:%M:%SZ)" \
             > /dev/null 2>&1
         fi
       fi
     }
     ...
     # — Bots —
     check_bot "devbot" "$HOME/.claude/channels/crew-devbot"
     check_bot "ops" "$HOME/.claude/channels/crew-ops"
     ```
   - `ALERT_CHAT_ID` is populated from `defaults.allowFrom[0]` in the registry (the owner's Telegram ID)

7. Generate `DEPLOY.md` with setup instructions for the target VPS:
   - Prerequisites (Claude Code, Bun, tmux or systemd)
   - How to copy the deploy directory to the VPS
   - How to set up `.env` files with bot tokens
   - For tmux mode: how to run `crew-launch.sh` and attach
   - For systemd mode: how to install units, enable, and start
   - How to install the health cron job
   - SSH hardening recommendations (disable password auth, use key-only, change default port)
   - Firewall recommendations (allow only SSH, deny all inbound by default, use `ufw` or `iptables`)
   - Recommendation to use a dedicated non-root user for the fleet

8. Output summary of generated files:
   ```
   DEPLOY FILES GENERATED
   =======================

   Directory:  ~/.claude/channels/crew-deploy/

   Scripts:
     crew-launch.sh          tmux launch script (chmod 755)
     crew-health-cron.sh     health monitor for crontab (chmod 755)

   Docs:
     DEPLOY.md               VPS setup and security guide

   Next steps:
   1. Review generated files in ~/.claude/channels/crew-deploy/
   2. Copy to your VPS: scp -r ~/.claude/channels/crew-deploy/ user@vps:~/.claude/channels/
   3. Follow DEPLOY.md for server setup
   ```

   If systemd mode was chosen, also list the `.service` files:
   ```
   Systemd units:
     crew-devbot.service     Bot: devbot
     crew-ops.service        Bot: ops
   ```

**Security:**
- Generated files do NOT contain tokens — they reference `.env` files at runtime
- systemd units use a placeholder for `CLAUDE_CODE_OAUTH_TOKEN` that the operator must replace
- `DEPLOY.md` includes SSH hardening and firewall recommendations
- `chmod 700` on the deploy directory, `chmod 755` on generated scripts
- Health cron script reads tokens from `.env` files at execution time, not at generation time

### `send <to> <message>`

Send a message from the current bot to another bot in the fleet.

1. **Validate sender:**
   - Read `TELEGRAM_STATE_DIR` from the environment to identify the current bot
   - Look up the sender in the registry — if not found, refuse: "This session is not associated with a registered bot."

2. **Validate recipient:**
   - Look up `<to>` in the registry — if not found, list available bots and refuse
   - Reject self-messaging: if `<to>` resolves to the same bot as the sender, refuse: "A bot cannot message itself."

3. **Validate message:**
   - `<message>` must not be empty
   - Enforce a 4KB (4096 bytes) size limit on the message body — refuse if exceeded
   - Strip any executable content: reject messages containing backticks, `$()`, `$(`, or `\`` patterns

4. **Create inbox directory (if needed):**
   ```bash
   mkdir -p ~/.claude/channels/crew-inbox/<to>
   chmod 700 ~/.claude/channels/crew-inbox/<to>
   ```

5. **Write message file:**
   - Generate a UUID (e.g., via `uuidgen` or equivalent)
   - Filename: `<timestamp>-<uuid>.json` where timestamp is ISO 8601 compact (e.g., `20260323T141500Z`)
   - Contents:
     ```json
     {
       "id": "<uuid>",
       ...
       "to": "<to>",
       "subject": "Message from <sender-bot-name>",
       "body": "<message>",
   ...
   - Apply permissions:
     ```bash
     chmod 600 ~/.claude/channels/crew-inbox/<to>/<filename>
     ```

6. **Confirm:** `Message sent to <to>.`

**Security notes:**
- No executable content allowed in message body (backticks, `$()` patterns rejected)
- No self-messaging — sender and recipient must be different bots
- 4KB size limit prevents abuse
- Inbox directories are `chmod 700`, message files are `chmod 600`

### Inbox Check (automatic)

Not a user command — this happens automatically on session start or periodically.

1. **Determine current bot name** from `TELEGRAM_STATE_DIR` and the registry
2. **Check inbox:** look for files in `~/.claude/channels/crew-inbox/<botname>/`
3. **If messages exist:**
   - Read each `.json` file, parse the message
   - Display to the user:
     ```
     INBOX — <botname>
     ==================
     From: <from>    Time: <timestamp>
     <body>
     ------------------
     ```
   - Create archive subdirectory if needed:
     ```bash
     mkdir -p ~/.claude/channels/crew-inbox/<botname>/archive
     chmod 700 ~/.claude/channels/crew-inbox/<botname>/archive
     ```
   - Move processed messages to `archive/`:
     ```bash
     mv ~/.claude/channels/crew-inbox/<botname>/<file> ~/.claude/channels/crew-inbox/<botname>/archive/
     ```
4. **If no messages:** silently continue (no output)
...
COMMANDS
  ...
  /solocrew deploy [--systemd|--tmux]  Generate VPS deployment scripts
  ...
  /solocrew send <to> <message>     Send a message to another bot
...
LIMITATIONS
  ...
  - Cross-bot messaging uses a file-based inbox system (~/.claude/channels/crew-inbox/) — delivery is asynchronous and only checked on session start or periodically, not real-time
